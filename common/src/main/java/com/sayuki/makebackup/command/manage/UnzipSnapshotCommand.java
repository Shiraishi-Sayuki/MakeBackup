...
// スナップショット解凍コマンドクラス - ZIPを展開する
public class UnzipSnapshotCommand extends ConfirmableSubCommand {

    private Target storage;
    private Snapshot backup;

    // コンストラクタ - 初期化する
    public UnzipSnapshotCommand(ModCommandSender sender, CommandArgs arguments) {
    ...
    public boolean check() {
        ...
        if (Snapshot.BackupFileType.DIR.equals(backup.getFileType())) {
            returnFailure("Snapshot is already unzipped!");
            return false;
        }
        ...
        if (!sender.hasPermission(Permissions.UNZIP.getPermission(storage))) {
    ...
    // 実行する - 解凍タスクを開始する
    public void run() {
        MakeBackup.getInstance().getTaskManager().startTaskAsync(backup.getUnZipTask(), sender, List.of(Permissions.UNZIP.getPermission(storage)));
    }
}
