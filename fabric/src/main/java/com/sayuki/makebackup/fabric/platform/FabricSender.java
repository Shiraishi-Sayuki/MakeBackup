...
// Fabric送信者クラス - コマンド送信者をラップする
public class FabricSender implements ModCommandSender {
    ...
    // 初期化する
    private FabricSender(CommandSourceStack source, ServerPlayer player) {
        this.source = source;
        this.player = player;
    }

    // コンソール送信者を作成する
    public static FabricSender console() {
        return new FabricSender(null, null);
    }

    // プレイヤー送信者を作成する
    public static FabricSender player(ServerPlayer player) {
        return new FabricSender(null, player);
    }

    // コマンドソースから作成する
    public static FabricSender source(CommandSourceStack source) {
        return new FabricSender(source, null);
    }
