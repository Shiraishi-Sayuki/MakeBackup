...
import com.sayuki.makebackup.command.ModCommandTree.ArgType;
import com.sayuki.makebackup.platform.ModCommandSender;
import net.minecraft.commands.CommandSourceStack;
import net.minecraft.commands.Commands;
import net.minecraft.server.level.ServerPlayer;
import com.sayuki.makebackup.fabric.platform.FabricSender;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

// コマンド登録クラス - Fabricのコマンドを登録する
public class FabricCommandRegistry {
    ...
    private static Map<String, Object> argumentValues(CommandContext<CommandSourceStack> context) {
        ...
        for (com.mojang.brigadier.context.ParsedCommandNode<CommandSourceStack> node : context.getNodes()) {
            if (node.getNode() instanceof com.mojang.brigadier.tree.ArgumentCommandNode<?, ?>) {
                ...
                Object value = context.getArgument(name, Object.class);
                args.put(name, unwrap(value));
            }
        }
        return args;
    }
    ...
    private static Object unwrap(Object value) {
        if (value instanceof CommandSyntaxException) return value;
        if (value instanceof String s) return stripQuotes(s);
        if (value instanceof Integer i) return i;
        if (value instanceof Long l) return l;
        if (value instanceof net.minecraft.commands.arguments.selector.EntitySelector) return null;
        return value;
    }
    ...
    private static ModCommandSender wrapSender(CommandSourceStack source) {
        ServerPlayer player = source.getPlayer();
        return player != null ? FabricSender.player(player) : FabricSender.source(source);
    }
}
